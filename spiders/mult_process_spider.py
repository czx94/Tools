...
def downloadImgFromBaiduimage(page, folderPath='baiduimage', keyword='python'):
    for i in range(page):
        w = urllib.request.quote(keyword)
        url = "https://image.baidu.com/search/flip?tn=baiduimage&word=" + w + "&pn=" + str(i * 20)
        html = getHtml(url)
        # ------ 利用正则表达式匹配网页内容找到图片地址 ------
...
if __name__ == '__main__':
    ...
    #搜索关键字并下载百度图片
    word = '皮卡丘'
    downloadImgFromBaiduimage(10, './baiduimage/'+word+'/',  word)
    # 下载百度帖子内所有图片
